...
class ChartDataComponent extends React.Component {
  ...
  homeSetEchart = () => {
    var myChart = echarts.init(document.getElementById("fchart3"),'walden');
  ...
  handleExcel = () => {
    api.post(apiPath.getUserRechargeList, {action: 2, userId: this.props.userId, page: this.state.startPageNum, size: 10*this.state.endPageNum})
        .then(function (response) {
          let res = response.data;
  ...
  render() {
    ...
    return (
        <div style={this.props.chartStyle} className="chart-data-view">
          ...
          <div id="fchart3" style={{ height: 300 }}></div>
        </div>
    );
  }
}
